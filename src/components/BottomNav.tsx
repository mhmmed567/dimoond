...
import { Coffee, ShoppingBag, Home } from 'lucide-react';
...
const BottomNav = () => {
  ...
  const hidePaths = ['/', '/admin', '/staff', '/login', '/cart', '/checkout', '/confirmation'];
  if (hidePaths.includes(location.pathname)) return null;

  return (
    <div className="fixed bottom-0 left-0 right-0 z-[100] flex justify-center pointer-events-none">
      <div className="w-full max-w-lg pointer-events-auto">
        <div className="bg-[#faf7f2]/80 backdrop-blur-2xl border-t border-[#2a1810]/5 px-6 pb-8 pt-4 flex justify-around items-center shadow-[0_-10px_40px_rgba(42,24,16,0.1)] md:rounded-t-[2.5rem] md:border-x">
        {navItems.map((item) => {
